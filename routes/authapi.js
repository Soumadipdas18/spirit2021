...
app.post("/register", async (req, res) => {
  ...
  try {
    ...
    if (error.code === 11000) {
      ...
      return res.json({status: "error",error: "vmro this email is already in use",});
    }
	  else if(error){
...
app.get("/verifyaccount/:id", async (req, res) => {
  const doc = await stuff.model.findById(req.params.id)
    .then((user) => {
      if (user != null) {
    ...
    .then(async (user) => {
      if (user != null) {
        ...
        if (await bcrypt.compare(plainTextPassword, user.password)) {
          return res.json({ status: "error", error: `Your new password is same as your old password` });
        }
